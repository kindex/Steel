	...
	public partial class Demo
	{
		private void LoadPhysics()
		{
			...
			_core = new Core( coreDesc, output );
				...
				_core.SetParameter( PhysicsParameter.VisualizeFluidEmitters, false ); // Slows down rendering
			...
			_scene = _core.CreateScene( new Vector3( 0, -9.81f, 0 ), true );
			...
			for( int x = 0; x < 5; x++ )
			{
				...
				ActorDescription actorDesc = new ActorDescription();
					...
					actorDesc.GlobalPose = Matrix.CreateTranslation( 100, 125 + 3 * x, 20 );
			...
			#region Cloth (Flag)
			{
				...
				ClothDescription clothDesc = new ClothDescription();
					...
					clothDesc.MeshData = new MeshData( grid.Points.Length, grid.Indices.Length / 3, 0, sizeof( float ) * 3, sizeof( float ) * 3, sizeof( int ), sizeof( int ) );
					...
					clothDesc.GlobalPose =
						Matrix.CreateRotationZ( Mathematics.PiOver2 ) *
						Matrix.CreateRotationX( Mathematics.PiOver2 ) *
				...
				ActorDescription flagPoleActorDesc = new ActorDescription();
					flagPoleActorDesc.Shapes.Add( new BoxShapeDescription( 1f, 20.0f, 1f ) );
			...
			#region Fluid
			{
				...
				ParticleData particleWriteData = new ParticleData();
					particleWriteData.AllocatePositionBuffer<Vector3>( maximumParticles );
				...
				FluidDescription fluidDesc = new FluidDescription();
					...
					fluidDesc.SetParticleWriteData( particleWriteData );
			...
			#region Force Field
			{
				...
				ForceFieldDescription forceFieldDesc = new ForceFieldDescription();
					forceFieldDesc.Constant = new Vector3( 0, 1000.0f, 0 );

				ForceField forceField = _scene.CreateForceField( forceFieldDesc );
					forceField.CreateShape( boxForceFieldShapeDesc );
					forceField.Pose = Matrix.CreateTranslation( 30, 5, 0 );
			}
			#endregion
				...
				SoftBodyDescription desc = new SoftBodyDescription();
					...
					desc.Flags |= SoftBodyFlag.Visualization;
					{
						MeshData data = new MeshData();
							data.AllocatePositions<Vector3>( vertices.Length );
							data.AllocateIndices<int>( tetrahedraSingles.Length );
						desc.MeshData = data;
					}
